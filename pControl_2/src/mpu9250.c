...
static mpu9250_status_t mpu9250_readBytes(const uint8_t regAddr, uint8_t *pData, uint8_t count);
...
mpu9250_status_t mpu9250_init( mpu9250_InitStruct* mpu9250_Init )
{
  ...
  tmpData = mpu9250_Init->SampleRate;
...
mpu9250_status_t mpu9250_readAccelData_int16(int16_t *pAccel)
{
  ...
  for(uint8_t i = 0; i < 3; i++)
    *pAccel++ = (int16_t) (((int16_t) rawData[2 * i] << 8) | rawData[2 * i + 1]);
...
static mpu9250_status_t mpu9250_isReady(void)
{
  ...
  if(ciaa_I2C_isReady(I2C0, devAddr++))
  {
    mpuAddr = MPU9250_ADDR;
    status = MPU9250_OK;
  }
  else if(ciaa_I2C_isReady(I2C0, devAddr))
  {
...
static mpu9250_status_t mpu9250_getStatus(void)
{
  ...
  status = (mpuStatus & MPU9250_INT_DATA_READY_MSK)?(MPU9250_OK):(MPU9250_ERROR);
...
static mpu9250_status_t mpu9250_writeReg(const uint8_t regAddr, uint8_t data)
{
  ...
  uint8_t aTxBuff[2] = {regAddr, data};
  uint8_t *pTxBuff = &aTxBuff[0];

  if(Chip_I2C_MasterSend(I2C0, mpuAddr, pTxBuff, 2))
    status = MPU9250_OK;
...
static mpu9250_status_t mpu9250_readReg(const uint8_t regAddr, uint8_t *pData)
{
  ...
  if(Chip_I2C_MasterCmdRead(I2C0, mpuAddr, regAddr, pData, 1))
    status = MPU9250_OK;
